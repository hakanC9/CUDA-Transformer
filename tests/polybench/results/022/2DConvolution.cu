...
__global__ void convolution2D_kernel(int ni, int nj, DATA_TYPE *A, DATA_TYPE *B)
{
	...
	//////// CUDA-TRANSFORMER WAS HERE : Choose one if else branch
{
			...
			DATA_TYPE POLYBENCH_2D(B_outputFromGpu, NI, NJ, ni, nj))
{
	...
	block.x -= 64;		//////// CUDA-TRANSFORMER WAS HERE : dim3 parameter changed 
	grid.x -= 0;		//////// CUDA-TRANSFORMER WAS HERE : dim3 parameter changed 
